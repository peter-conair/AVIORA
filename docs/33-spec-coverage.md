...
| **Scheduled work happens**                                                                              | A scheduler runs renewals, rank evaluation, commission drafts and the webhook sweep on a timer, one row per occurrence (docs/35). One process, in-process interval. | A second API instance to prove the claim across processes, and an alert on runs left `claimed` — today an operator has to look (docs/35 §5). |
