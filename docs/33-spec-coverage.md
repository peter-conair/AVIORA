...
| **The outbox survives production load**                                                                 | Measured 2026-08-21: 161 events/s with handlers, 2,400/s empty. Found and removed a throttle that capped the DEPLOYED rate at 10/s (docs/41). Two relays race one backlog and each handler still runs once.                                 | A real production load profile, and narrowing the transaction that currently spans a whole batch of handler I/O (docs/41 §3).                                 |
