...
6. **Output filter.** Implemented in `modules/ai/output-safety.ts` (docs/50): a sentence-level pattern check in English **and Thai** for diagnosis language, treat/cure/prevent claims and medication instructions. A hit REPLACES the answer with a plain refusal rather than editing it — an answer quietly rewritten to look safe is one nobody can audit. It is a backstop for a model slip, not a defence against a determined jailbreak, and docs/50 §1 says so rather than letting anyone relax the prompt in front of it.
7. **Auditability.** Every block writes `ai.safety.blocked` with the rules that hit and the agent that produced it — the only way to answer "how often does this happen, and is it getting worse". The safety prompt layer is versioned in the repo so
