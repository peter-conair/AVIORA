...
| 16  | AI respects tenant and team permissions        | ✅        | Tenant scope: `knowledge-ai.e2e` (retrieval through the tenant-scoped service, conversations per member, cross-member reads 404). Team scope: `team-knowledge.e2e` (docs/37) — a member of one branch asks the assistant a question the other branch's playbook would answer, and neither the retrieval nor the citations contain it. Scope is applied IN the query, so nothing is filtered after the fact. |
...
**Remaining before MVP can be called done:** nothing in this table. Row 16 was
the last one open and closed in Sprint 18 (docs/37): team-scoped knowledge now
exists, and the assistant respects it because retrieval never loads an article
the caller may not read.

Row 18 stays marked ⚠️ manual by choice. The Playwright suite does now assert
mobile width, but "usable" is a judgement a viewport check cannot make, and
marking it green on the strength of an automated scroll-width assertion would
claim more than the test proves.
